...
############################################################
#  Tensorboard for training and validation in different files
############################################################
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    if augment:
        ...
        # Imgaug needs labels be coded as ints from 0 to n where 0 is background
        # .arr form imgaug return the one hot-encoded augmented mask
        # Masks are loaded in one-hot encoded format without backgorund so we need to convert
        mask_imgaug = np.argmax(np.append(np.zeros((mask.shape[0], mask.shape[1], 1)), mask, axis=2), axis=2)
        segmap = SegmentationMapOnImage(mask_imgaug, nb_classes=mask.shape[2]+1, shape=image.shape)
        image, segmap_aug = train_seq(image=image, segmentation_maps=segmap)
        mask = segmap_aug.arr[:, :, 1:]
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        if config.GPU_COUNT > 1:
            from models.Mask_RCNN.parallel_model import ParallelModel
